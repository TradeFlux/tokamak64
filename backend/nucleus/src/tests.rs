...
mod tests {
    ...
    #[test]
    fn speed_bonus_increases_with_time() {
        let mut charge = make_charge(100, ElementIndex(0), 0);
        charge.timestamp = 1000;

        let bonus_early = speed_bonus(&charge, 1010);
        let bonus_late = speed_bonus(&charge, 2000);

        assert!(bonus_late > bonus_early);
    }

    #[test]
    fn speed_bonus_caps_at_max_delta() {
        let mut charge = make_charge(100, ElementIndex(0), 0);
        charge.timestamp = 0;

        let bonus_at_max = speed_bonus(&charge, MAX_DELTA_TIMESTAMP + 100);
        let bonus_at_cap = speed_bonus(&charge, MAX_DELTA_TIMESTAMP);

        // Beyond MAX_DELTA_TIMESTAMP, bonus should be same (capped)
        assert_eq!(bonus_at_max, bonus_at_cap);
    }
